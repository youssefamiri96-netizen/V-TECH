...
def groupage_mail_rows_for_shipments(shipments: list[str]) -> list[dict[str, Any]]:
    ...
    rows = rows_for_shipments(selected_shipments)
    row_by_shipment = {
    ...
    missing_shipments = [
        shipment for shipment in selected_shipments
        if shipment not in row_by_shipment
    ]
    invalid_rows = [
        row for row in rows
        if clean_text(row.get("Stato")) != STATUS_PLANNED or not is_brt_groupage(row)
    ]
    if missing_shipments or invalid_rows:
        invalid_shipments = [
            clean_text(row.get("Shipment"))
            for row in invalid_rows
            if clean_text(row.get("Shipment"))
        ]
        invalid = ", ".join([*missing_shipments, *invalid_shipments])
        raise ValueError(f"La mail Groupage si puo generare solo dalle spedizioni groupage pianificate selezionate. Controlla: {invalid}")
...
def mail_total_volume_m3(rows: list[dict[str, Any]]) -> float:
    return round(sum(to_float(row.get("Grand Total Shipment Ftp Vol m3")) or 0 for row in rows), 3)


def build_mail_weight_total_row(rows: list[dict[str, Any]], columns: list[str]) -> str:
    weight_column = "Grand Total Shipment Ftp Wgt Kg"
    volume_column = "Grand Total Shipment Ftp Vol m3"
    if weight_column not in columns and volume_column not in columns:
        return ""

    def total_row(target_column: str, label: str, total: float) -> str:
        cells = []
        for index, column in enumerate(columns):
            if column == target_column:
                cells.append(f"<td><strong>{html_lib.escape(format_number(total))}</strong></td>")
            elif index == 0:
    ...
    if weight_column in columns:
        footer_rows.append(total_row(weight_column, "Totale peso con bancali", mail_total_weight_kg(rows)))
    if volume_column in columns:
        footer_rows.append(total_row(volume_column, "Totale volume con bancali", mail_total_volume_m3(rows)))
    return "".join(footer_rows)


def build_groupage_mail_html(rows: list[dict[str, Any]], mail_date: date, mail_token: str = "") -> str:
...
<html>
  ...
  <body>
    <!-- VTECH-GROUPAGE-MAIL-ID:{html_lib.escape(mail_token)} -->
    <p>Buongiorno,<br>
...
def build_table_plain_text(rows: list[dict[str, Any]], columns: list[str], include_weight_total: bool = False) -> str:
    ...
    if include_weight_total and weight_column in columns:
        ...
        total_cells[0] = "Totale peso con bancali"
        total_cells[columns.index(weight_column)] = format_number(mail_total_weight_kg(rows))
        body.append("\t".join(total_cells))
    volume_column = "Grand Total Shipment Ftp Vol m3"
    if include_weight_total and volume_column in columns:
        total_cells = [""] * len(columns)
        total_cells[0] = "Totale volume con bancali"
        total_cells[columns.index(volume_column)] = format_number(mail_total_volume_m3(rows))
        body.append("\t".join(total_cells))
    return "\n".join([header, *body])


def build_groupage_mail_text(rows: list[dict[str, Any]], mail_date: date) -> str:
    columns = groupage_mail_columns()
    total_pallets = sum(ceil_pallets(row.get("Theoretical Pallets")) for row in rows)
    return "\n\n".join(
        [
            "Buongiorno,\ndi seguito il dettaglio delle spedizioni che caricherete con il ritiro n°     per il cliente Vtech:",
            f"Totale bancali da ritirare: {total_pallets}",
...
def create_groupage_mail(shipments: list[str]) -> dict[str, Any]:
    rows = groupage_mail_rows_for_shipments(shipments)
    mail_date = date.today()
    mail_token = f"VTECH-GROUPAGE-{uuid.uuid4().hex}"
    html_body = build_groupage_mail_html(rows, mail_date, mail_token)
    text_body = build_groupage_mail_text(rows, mail_date)
    ...
    record_pending_groupage_mail(mail_token, rows, mail_date, subject)
    return {
        "rows": len(rows),
        "pallets": sum(ceil_pallets(row.get("Theoretical Pallets")) for row in rows),
        "date": mail_date.strftime("%d/%m/%Y"),
        "pending": True,
...
class VTechWebHandler(BaseHTTPRequestHandler):
    ...
    def _handle_groupage_mail(self, body: dict[str, Any], user: dict[str, str]) -> None:
        ...
        result = create_groupage_mail(shipments)
    ...
    def _handle_action(self, body: dict[str, Any], user: dict[str, str]) -> None:
        ...
        if action == "planned":
            planned_at = clean_text(body.get("plannedAt")) or None
            for shipment in shipments:
                mark_planned(shipment, planned_at=planned_at)
        elif action == "planned_date":
            planned_at = body.get("plannedAt")
            for shipment in shipments:
                update_planned_date(shipment, planned_at)
        elif action == "unplanned":
            for shipment in shipments:
                mark_unplanned(shipment)
        elif action == "delivered":
            xml_paths: list[str] = []
            delivered_at = clean_text(body.get("deliveredAt")) or None
            for shipment in shipments:
                _delivered_at, shipment_xml_paths = mark_delivered(shipment, delivered_at=delivered_at)
                xml_paths.extend(str(path) for path in shipment_xml_paths)
            ...
            departed_at = manual_departed_at or date.today().strftime("%Y-%m-%d")
            for shipment in shipments:
