...
def pallet_weight_breakdown(row: dict[str, Any] | None) -> tuple[float, int, int, str]:
    if not row:
        return 0.0, 0, 0, ""
    ...
    return pallet_weight, epal_count, disposable_count, reason
...
def apply_pallet_weight_to_shipment(row: dict[str, Any]) -> None:
    ...
    pallet_weight, epal_count, disposable_count, reason = pallet_weight_breakdown(row)
    ...
    details: list[str] = []
    if epal_count:
        details.append(f"{epal_count} EPAL x {BANCALI_EPAL_WEIGHT_KG:.0f} kg")
    if disposable_count:
        details.append(f"{disposable_count} a perdere x {BANCALI_A_PERDERE_WEIGHT_KG:.0f} kg")
