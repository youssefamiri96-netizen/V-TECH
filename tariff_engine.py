...
class ActiveRateCard:
    ...
    def contract_sla(self, row: dict[str, Any]) -> ContractSlaResult:
        ...
        early_delivery_at = parse_datetime_value(row.get("Early Delivery Date"))
        breaches: list[str] = []
        ...
        if early_delivery_at is None:
            missing_dates.append("Early Delivery Date")
        elif early_delivery_at.date() < min_delivery_at.date():
            breaches.append(
                f"Early delivery {early_delivery_at.strftime('%d/%m/%Y')} prima del minimo {min_delivery_at.strftime('%d/%m/%Y')}"
            )
...
def apply_tariffs_to_shipments(
    ...
    for row in shipment_rows:
        ...
        if passive_results:
            ...
            row["Miglior Vettore"] = f"{chosen.carrier} EUR {chosen.cost:.2f}"
            if len(passive_results) > 1:
                row["Secondo Vettore"] = f"{passive_results[1].carrier} EUR {passive_results[1].cost:.2f}"
            if len(passive_results) > 2:
                row["Terzo Vettore"] = f"{passive_results[2].carrier} EUR {passive_results[2].cost:.2f}"
