...
BANCALI_EPAL_VOLUME_M3 = 1.20 * 0.80 * 0.144
BANCALI_A_PERDERE_VOLUME_M3 = BANCALI_EPAL_VOLUME_M3
...
def pallet_support_breakdown(row: dict[str, Any] | None) -> tuple[float, float, int, int, str]:
    ...
    pallet_weight = (epal_count * BANCALI_EPAL_WEIGHT_KG) + (
        disposable_count * BANCALI_A_PERDERE_WEIGHT_KG
    )
    pallet_volume = (epal_count * BANCALI_EPAL_VOLUME_M3) + (
        disposable_count * BANCALI_A_PERDERE_VOLUME_M3
    )
...
def apply_pallet_weight_to_shipment(row: dict[str, Any]) -> None:
    ...
    if goods_volume is not None:
        total_volume = round(goods_volume + pallet_volume, 3)
    ...
    details: list[str] = []
    if epal_count:
        details.append(f"{epal_count} EPAL x {BANCALI_EPAL_WEIGHT_KG:.0f} kg / {BANCALI_EPAL_VOLUME_M3:.3f} m3")
    if disposable_count:
        details.append(f"{disposable_count} a perdere x {BANCALI_A_PERDERE_WEIGHT_KG:.0f} kg / {BANCALI_A_PERDERE_VOLUME_M3:.3f} m3")
...
class BrtPassiveRateCard:
    ...
    def calculate(
        ...
        if appointment_auto and not flag_enabled(shipment_row, flags, "Appuntamento"):
