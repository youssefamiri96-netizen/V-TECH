...
const pageMeta = {
  ...
  groupage: {
    title: "Groupage",
    subtitle: "Mail BRT, carico e storico operativo groupage",
    sections: [
      ["plannedGroupage", "Da mandare in mail BRT"],
      ["departed", "Groupage caricato"],
    ],
  },
  ftl: {
    title: "FTL",
    subtitle: "Richieste vettore, conferme scarico, consegna e XML",
    sections: [
      ["plannedCustomer", "FTL pianificato / richiesta inviata"],
      ["confirmedFtl", "FTL confermati"],
...
function render() {
  ...
  els.metrics.hidden = isBilling || isRegistry || isAdmin || isDashboard;
  els.detailPanel.hidden = isBilling || isRegistry || isAdmin || isDashboard;
  document.querySelector("#importBtn").hidden = Boolean(state.data.auth?.billingOnly);
  renderSidebarStats();
  renderHeroTrend();
  renderMetrics();
  if (isDashboard) {
...
function renderDashboard() {
  ...
  const towerHealth = verifyRows.length
    ? ["ATTENZIONE", "Hai ordini da verificare prima di pianificare", "hot"]
    : planningRows.length
      ? ["OPERATIVA", "La verifica e pulita, lavora sulla pianificazione", "ready"]
      : ["PULITA", "Nessuna urgenza operativa in coda", "clear"];
  els.content.innerHTML = `
    <section class="dashboard-page">
      <div class="tower-status">
        <article class="tower-status-main ${towerHealth[2]}">
          <span>Stato control tower</span>
          <strong>${escapeHtml(towerHealth[0])}</strong>
          <small>${escapeHtml(towerHealth[1])}</small>
        </article>
        <article>
          <span>Report caricato</span>
          <strong>${escapeHtml(report.name || "Nessun report")}</strong>
          <small>${escapeHtml(report.updatedAt || "Carica un report da Pianificazione")}</small>
        </article>
        <article>
          <span>Margine mese</span>
          <strong>${escapeHtml(kpis.monthMargin || moneyText(monthSummary.margin))}</strong>
          <small>Attivo ${escapeHtml(moneyText(monthSummary.active))} / Passivo ${escapeHtml(moneyText(monthSummary.passive))}</small>
        </article>
      </div>
      ...
      <div class="tower-grid">
        ...
        <aside class="tower-next">
          <div class="tower-section-head compact">
            <div>
              <span>Prossime azioni</span>
              <h2>Lavora in questo ordine</h2>
            </div>
          </div>
          <ol>
            <li class="${verifyRows.length ? "active" : ""}"><b>1</b><span>Verifica wave e dati mancanti</span><strong>${verifyRows.length}</strong></li>
            <li class="${!verifyRows.length && planningRows.length ? "active" : ""}"><b>2</b><span>Pianifica groupage e FTL/LTL</span><strong>${planningRows.length}</strong></li>
            <li><b>3</b><span>Invia mail BRT / vettori</span><strong>${mailRows.length + ftlRows.length}</strong></li>
            <li class="${slaRows.length ? "danger" : ""}"><b>!</b><span>Controlla SLA fuori contratto</span><strong>${slaRows.length}</strong></li>
          </ol>
        </aside>
      </div>
...
function renderPlanningWorkspace() {
  ...
  const totalRows = sections.reduce((total, section) => total + section.rows.length, 0);
  const urgentRows = [
    ...(state.data.groups.verify || []),
    ...(state.data.groups.slaBreaches || []),
  ].filter((row, index, list) => list.findIndex(item => item.shipment === row.shipment) === index);
  els.content.innerHTML = `
    <section class="planning-workspace tower-planning">
      <div class="planning-command simple">
        <div>
          <span>Control tower planning</span>
          <h2>Una spedizione, una scheda, una decisione</h2>
          <p>Leggi cliente, indirizzo, wave, date, merce, vettore e margine senza scroll laterale. Spunta le schede e usa le azioni sopra.</p>
        </div>
        <div class="planning-command-kpis">
          <article><strong>${rowsCount("verify")}</strong><span>da verificare</span></article>
          <article><strong>${rowsCount("openGroupage") + rowsCount("openDirect")}</strong><span>da pianificare</span></article>
          <article><strong>${urgentRows.length}</strong><span>alert</span></article>
        </div>
      </div>
      <div class="planning-switchboard">
        ${sections.map(section => `
          <a href="#lane-${escapeHtml(section.key)}" style="--accent:${planningSectionAccent(section.key)}">
            <span>${escapeHtml(planningNextLabel(section.key))}</span>
            <strong>${escapeHtml(section.title)}</strong>
