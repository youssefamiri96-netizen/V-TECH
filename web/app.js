...
const pageMeta = {
  dashboard: {
    title: "Buongiorno",
    subtitle: "Ecco la situazione operativa di oggi",
    sections: [],
  },
  planning: {
  ...
  groupage: {
    title: "Groupage",
    subtitle: "Comunicazioni Bartolini da preparare e monitorare",
    sections: [
...
const metricConfig = [
  ["monthActive", "Fatturato attivo", "FA", "vs ieri", "#2f8cff"],
  ["monthPassive", "Costi passivi", "CP", "vs ieri", "#9b5cff"],
  ["monthGrossMargin", "Margine lordo", "ML", "margine mese", "#26d0a1"],
  ["total", "Spedizioni totali", "ST", "groupage + FTL", "#f59e0b"],
  ["alerts", "Alert attivi", "AA", "da gestire", "#ef4444"],
  ["monthMargin", "Margine stimato", "MS", "attivo meno passivo", "#14b8a6"],
];
...
function applyData(data, { renderPage = true, keepSelection = true } = {}) {
  ...
  if (els.currentUser) {
    ...
    const displayName = user.displayName || user.username || "Utente";
    const initials = displayName
      ...
      .join("")
      .toUpperCase() || "YS";
    els.currentUser.parentElement?.style.setProperty("--user-initials", `"${initials}"`);
    els.currentUser.innerHTML = `<strong>${escapeHtml(displayName)}</strong><small>${escapeHtml(roleLabel)}</small>`;
  }
...
function render() {
  ...
  if (els.todayChip) {
    els.todayChip.textContent = `Oggi, ${longTodayLabel()}`;
  }
  const displayName = state.data.auth?.user?.displayName || state.data.auth?.user?.username || "";
  els.pageTitle.textContent = isDashboard && displayName ? `Buongiorno, ${displayName}` : meta.title;
...
function metricValue(key, kpis) {
  const monthSummary = summarizeBilling(
    billingRows().filter(item => item.month === monthKey(new Date()))
  );
  if (key === "monthActive") return moneyText(monthSummary.active);
  if (key === "monthPassive") return moneyText(monthSummary.passive);
  if (key === "monthGrossMargin") return moneyText(monthSummary.margin);
  if (key === "monthMargin") return moneyText(monthSummary.margin);
  if (key === "alerts") return String((kpis.slaBreaches || 0) + (kpis.verify || 0));
  if (key === "loadedMonth") {
...
function metricHint(key, fallback) {
  if (key === "monthGrossMargin") {
    const monthSummary = summarizeBilling(
      billingRows().filter(item => item.month === monthKey(new Date()))
    );
    const ratio = monthSummary.active ? (monthSummary.margin / monthSummary.active) * 100 : 0;
    return `${compactNumberText(ratio, 1)}% di margine`;
  }
...
function renderDashboard() {
  ...
  const loadedSummary = monthLoadedSummary();
  const todoCards = [
    ["Da verificare", verifyRows.length, "Wave da verificare", "#2f8cff", "planning", verifyRows],
    ["Da pianificare", planningRows.length, "Groupage + FTL/LTL aperti", "#2563eb", "planning", planningRows],
    ["Spedizioni caricate", `${loadedSummary.count} / ${compactNumberText(loadedSummary.pallets)} plt`, "Questo mese", "#8b5cf6", "billing", []],
  ...
  els.content.innerHTML = `
    <section class="dashboard-page">
      ...
      <div class="tower-grid">
        <section class="tower-primary">
          <div class="tower-section-head">
            <div>
              <span>Priorita operative</span>
              <h2>Ordini che richiedono attenzione</h2>
            </div>
      ...
      <div class="quick-actions-grid">
        <button type="button" data-dashboard-page="planning"><strong>Nuova spedizione</strong><span>Crea un nuovo ordine</span></button>
        <button type="button" data-dashboard-page="planning"><strong>Assegna vettore</strong><span>Ricerca e assegna vettore</span></button>
        <button type="button" data-dashboard-page="ftl"><strong>FTL Diretta</strong><span>Crea spedizione diretta</span></button>
        <button type="button" data-dashboard-page="ftl"><strong>Scarichi oggi</strong><span>Visualizza scarichi</span></button>
        <button type="button" data-dashboard-page="planning"><strong>Alert SLA</strong><span>Visualizza criticita</span></button>
        <button type="button" data-dashboard-page="billing"><strong>Report mensile</strong><span>Analisi dettagliata</span></button>
      </div>
    </section>
  `;
}
...
async function performAction(action, extra = {}) {
  ...
  await reloadAfterMutation(shipments, { keepSelection: ["carrier", "service_level", "freight_code", "required_delivery_date", "manual_passive", "manual_pallets", "unload_date", "unload_booking", "active_urgent"].includes(action) });
