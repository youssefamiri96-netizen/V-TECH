...
function syncBulkCheckboxes() {
  document.querySelectorAll(".shipment-section").forEach(section => {
    ...
    if (!bulk) return;
    const shipments = [...section.querySelectorAll("tbody tr[data-shipment]")]
...
function allRows() {
  return Object.values(state.data?.groups || {}).flat();
}
...
els.content.addEventListener("click", (event) => {
  ...
  if (bulkCheck) {
    ...
    const shipments = section
      ? [...section.querySelectorAll("tbody tr[data-shipment]")]
          .map(row => row.dataset.shipment)
          .filter(Boolean)
      : [];
...
document.querySelector("#selectAllBtn").addEventListener("click", () => {
  ...
  meta.sections.forEach(([key]) => {
    (state.data.groups[key] || []).filter(rowMatches).forEach(row => state.selected.add(row.shipment));
  });
  renderSections();
  updateSelection();
});
