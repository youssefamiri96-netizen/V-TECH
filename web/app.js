...
function projectItalyPoint(lon, lat) {
  return {
    x: ((lon - 6.45) / (18.75 - 6.45)) * 100,
    y: ((47.10 - lat) / (47.10 - 36.65)) * 100,
  };
}
    ...
    <section class="geo-control-card">
      ...
      <div class="geo-body">
        <div class="italy-map-wrap">
          <svg class="italy-map" viewBox="0 0 100 100" role="img" aria-label="Mappa spedizioni per provincia">
            <defs>
            ...
            <path class="italy-mainland" d="M31 8 C38 3 48 8 50 17 C52 26 49 33 55 39 C62 45 70 47 70 55 C70 61 64 63 61 58 C58 53 54 51 49 54 C43 57 39 54 41 48 C43 41 37 39 34 33 C29 25 25 15 31 8Z" />
            <path class="italy-mainland" d="M55 54 C61 59 71 62 78 70 C84 77 86 86 82 91 C78 96 71 90 69 83 C67 77 60 73 55 69 C50 65 48 59 55 54Z" />
            <path class="italy-island" d="M20 55 C25 49 32 53 31 62 C30 71 26 80 20 79 C14 78 13 63 20 55Z" />
            <path class="italy-island" d="M58 84 C66 80 80 82 84 88 C78 94 64 94 56 90 C53 88 54 86 58 84Z" />
            ${mapped.map(item => {
              const radius = 2.7 + (item.count / maxCount) * 4.6;
              return `
                <g class="geo-point" style="--geo-accent:${item.count === maxCount ? "#0f766e" : "#0ea5e9"}">
                  <circle class="geo-point-pulse" cx="${item.point.x.toFixed(2)}" cy="${item.point.y.toFixed(2)}" r="${(radius * 2.1).toFixed(2)}" />
                  <circle class="geo-point-dot" cx="${item.point.x.toFixed(2)}" cy="${item.point.y.toFixed(2)}" r="${radius.toFixed(2)}" />
                  <text x="${(item.point.x + radius + 1).toFixed(2)}" y="${(item.point.y + 1.2).toFixed(2)}">${escapeHtml(item.province)}</text>
                  <title>${escapeHtml(`${item.province}: ${item.count} spedizioni, ${compactNumberText(item.pallets)} bancali`)}</title>
                </g>
              `;
            }).join("")}
          </svg>
          ...
          ${data.length ? data.slice(0, 7).map(item => `
            <div class="geo-rank-row">
              <div>
                ...
                <span>${item.count} spedizion${item.count === 1 ? "e" : "i"} · ${escapeHtml(compactNumberText(item.pallets))} bancali</span>
              </div>
