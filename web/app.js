...
const ITALY_MAP_SIZE = { width: 340, height: 320 };
const ITALY_MAP_BOUNDS = { west: 6.35, east: 19.15, north: 47.2, south: 36.9 };
const ITALY_MAP_PADDING = { x: 8, y: 4 };
...
function projectItalyPoint(lon, lat) {
  const drawableWidth = ITALY_MAP_SIZE.width - ITALY_MAP_PADDING.x * 2;
  const drawableHeight = ITALY_MAP_SIZE.height - ITALY_MAP_PADDING.y * 2;
  return {
    x: ITALY_MAP_PADDING.x + ((lon - ITALY_MAP_BOUNDS.west) / (ITALY_MAP_BOUNDS.east - ITALY_MAP_BOUNDS.west)) * drawableWidth,
    y: ITALY_MAP_PADDING.y + ((ITALY_MAP_BOUNDS.north - lat) / (ITALY_MAP_BOUNDS.north - ITALY_MAP_BOUNDS.south)) * drawableHeight,
  };
}
      ...
      <div class="geo-body">
        <div class="italy-map-wrap">
          <svg class="italy-map" viewBox="0 0 ${ITALY_MAP_SIZE.width} ${ITALY_MAP_SIZE.height}" role="img" aria-label="Mappa spedizioni per provincia">
            ...
            <image class="italy-map-image" href="/italy-map-outline.svg?v=fixed-base-20260522" x="0" y="0" width="${ITALY_MAP_SIZE.width}" height="${ITALY_MAP_SIZE.height}" preserveAspectRatio="xMidYMid meet" />
            ${mapped.map(item => {
              const percent = totalShipments ? item.count / totalShipments * 100 : 0;
              const radius = 4.2 + Math.sqrt(item.count / maxCount) * 9.8;
              const haloRadius = radius + 7 + Math.sqrt(item.pallets / Math.max(1, maxPallets)) * 10;
              const labelOffset = GEO_LABEL_OFFSETS[item.province];
              const labelDx = labelOffset?.[0] ?? (item.point.x > ITALY_MAP_SIZE.width - 42 ? -18 : 10);
              const labelDy = labelOffset?.[1] ?? 5;
              const labelAnchor = labelOffset?.[2] ?? (item.point.x > ITALY_MAP_SIZE.width - 42 ? "end" : "start");
              return `
