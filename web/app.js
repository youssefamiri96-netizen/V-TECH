...
function renderPlanningWorkspace() {
  ...
  els.content.innerHTML = `
    <section class="planning-workspace tower-planning">
      ...
      ${totalRows ? `
        ${verifySection ? renderPlanningLane(verifySection.key, verifySection.title, verifySection.rows) : ""}
        <div class="planning-mode-title"><span>Groupage</span><strong>BRT da pianificare o gia pronto per mail</strong></div>
        ${groupageSections.map(section => renderPlanningLane(section.key, section.title, section.rows)).join("")}
        <div class="planning-mode-title"><span>FTL / Dirette</span><strong>Vettore, costo, scarico e conferma</strong></div>
        ${ftlSections.map(section => renderPlanningLane(section.key, section.title, section.rows)).join("")}
      ` : `<div class="empty big-empty">Nessuna spedizione da pianificare con i filtri attuali.</div>`}
    </section>
  `;
  syncBulkCheckboxes();
}
...
function updateSelection() {
  ...
  if (els.actionMenuCount) els.actionMenuCount.textContent = String(count);
