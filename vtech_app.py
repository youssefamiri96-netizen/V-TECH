...
DATA_ACTIVE_RATES_PATH = DATA_DIR / "uploads" / "active_rates.xlsx"
SEED_REFRESH_WHEN_NEWER = {Path("uploads") / "active_rates.xlsx"}
...
def ensure_seed_data_files() -> None:
    ...
    if SEED_DIR.exists():
        for source in SEED_DIR.rglob("*"):
            ...
            refresh_from_seed = False
            if target.exists() and relative in SEED_REFRESH_WHEN_NEWER:
                try:
                    ...
                    refresh_from_seed = False
            if not target.exists() or refresh_from_seed:
...
COLUMN_TITLES = {
    ...
    "Peso Merce Kg": "Peso merce kg",
    "Peso Bancali Kg": "Peso bancali kg",
    "Peso Totale con Bancali Kg": "Peso totale kg",
    "Dettaglio Peso Bancali": "Dettaglio bancali",
    "Grand Total Shipment Ftp Wgt Kg": "Peso totale kg",
    "Volume Merce m3": "Volume merce m3",
    "Volume Bancali m3": "Volume bancali m3",
    "Volume Totale con Bancali m3": "Volume totale m3",
    "Grand Total Shipment Ftp Vol m3": "Volume totale m3",
    "Costo Attivo": "Attivo",
...
def required_iso_date_text(value: Any, label: str) -> str:
    parsed = parse_date_value(value)
    if parsed is None:
        raise ValueError(f"Inserisci una data {label} valida.")
    return parsed.strftime("%Y-%m-%d")
...
def display_value(row: dict[str, Any], column: str) -> str:
    ...
    if column in {
        ...
        "Peso Merce Kg",
        "Peso Bancali Kg",
        "Peso Totale con Bancali Kg",
        "Volume Merce m3",
        "Volume Bancali m3",
        "Volume Totale con Bancali m3",
    }:
...
def shipments_cache_key(db_path: Path = DB_PATH) -> tuple[Any, ...]:
    ...
    if not active_path.exists() and DATA_ACTIVE_RATES_PATH.exists():
        active_path = DATA_ACTIVE_RATES_PATH
...
def update_planned_date(shipment: str, planned_at: Any, db_path: Path = DB_PATH) -> None:
    planned_value = required_iso_date_text(planned_at, "partenza pianificata")
    init_db(db_path)
    with sqlite3.connect(db_path) as conn:
        cursor = conn.execute(
            "UPDATE shipments SET planned_at = ? WHERE shipment = ?",
            (planned_value, shipment),
        )
...
def delivery_order_numbers(row: dict[str, Any]) -> list[str]:
    orders_text = clean_text(row.get("Orders"))
    if not orders_text:
        raise ValueError("Ordine mancante: impossibile creare XML consegna.")

    orders: list[str] = []
    for part in re.split(r"\s*\|\s*|[,;\r\n]+", orders_text):
        order = clean_text(part)
...
def safe_xml_token(value: Any) -> str:
    token = clean_text(value)
    safe = "".join(char if char.isalnum() or char in "-_" else "_" for char in token)
    return safe or "ordine"


def generate_delivery_xml(row: dict[str, Any], delivered_at: str) -> list[Path]:
    XML_DIR.mkdir(parents=True, exist_ok=True)
    shipment = clean_text(row.get("Shipment"))
    if not shipment:
        raise ValueError("Shipment mancante: impossibile creare XML consegna.")
    orders = delivery_order_numbers(row)
    if not DELIVERY_XML_TEMPLATE_PATH.exists():
        raise FileNotFoundError(f"Modello XML non trovato: {DELIVERY_XML_TEMPLATE_PATH}")

    output_path = XML_DIR / f"KN_{safe_xml_token(shipment)}.xml"

    tree = ET.parse(DELIVERY_XML_TEMPLATE_PATH)
    root = tree.getroot()
    body = root.find("./TransmissionBody")
    if body is None:
        raise ValueError("Modello XML consegna non valido: TransmissionBody mancante.")
    template_elements = body.findall("GLogXMLElement")
    template_element = next(
        (element for element in template_elements if element.find("./TransOrderStatus") is not None),
        None,
    )
    if template_element is None:
        raise ValueError("Modello XML consegna non valido: GLogXMLElement/TransOrderStatus mancante.")

    for element in template_elements:
        body.remove(element)

    delivery_date = parse_date_value(delivered_at) or date.today()
    event_text = f"{delivery_date.strftime('%Y%m%d')}180000"
    for order_number in orders:
        order_element = copy.deepcopy(template_element)
        release_xid = order_element.find("./TransOrderStatus/ReleaseGid/Gid/Xid")
        event_date = order_element.find("./TransOrderStatus/EventDt/GLogDate")
        ...
        release_xid.text = order_number
        event_date.text = event_text
        body.append(order_element)
...
def mark_delivered(
    ...
    delivered_value = (
        required_iso_date_text(delivered_at, "scarico avvenuto")
        if clean_text(delivered_at)
    ...
    xml_paths = generate_delivery_xml(row, delivered_value)
    init_db(db_path)
    with sqlite3.connect(db_path) as conn:
        conn.execute(
            "UPDATE shipments SET status = ?, delivered_at = ?, delivery_xml_path = ? WHERE shipment = ?",
            (STATUS_DELIVERED, delivered_value, " | ".join(str(path) for path in xml_paths), shipment),
        )
    return delivered_value, xml_paths
...
def billing_order_numbers(row: dict[str, Any]) -> list[str]:
    try:
        return delivery_order_numbers(row)
    except ValueError:
        return [clean_text(row.get("Orders"))]


def split_billing_value(value: Any, index: int, count: int, decimals: int = 2) -> float | str:
    number = to_float(value)
    if number is None:
        return ""
    if count <= 1:
        return round(number, decimals)
    base = round(number / count, decimals)
    if index < count - 1:
        return base
    return round(number - base * (count - 1), decimals)


def billing_group_fill(index: int) -> PatternFill:
    colors = ("FFFFFF", "F8FBFF")
    return PatternFill("solid", fgColor=colors[index % len(colors)])


def apply_billing_group_fills(sheet, groups: list[tuple[int, int, PatternFill]]) -> None:
    for start_row, end_row, fill in groups:
...
def export_passive_billing_by_carrier(
    ...
    for carrier, carrier_rows in sorted(by_carrier.items()):
        ...
        for group_index, (ref_date, row) in enumerate(carrier_rows):
            ...
            for order_index, order_number in enumerate(orders):
                sheet.append([
                    ...
                    carrier,
                    clean_text(row.get("Freight Code")),
                    split_billing_value(row.get("Pallet Fatturati"), order_index, order_count),
                    split_billing_value(row.get("Grand Total Shipment Ftp Wgt Kg"), order_index, order_count),
                    split_billing_value(row.get("Grand Total Shipment Ftp Vol m3"), order_index, order_count),
                    split_billing_value(row.get("Costo Passivo Base BRT"), order_index, order_count),
                    *[
                        split_billing_value(extra_amounts.get(label, ""), order_index, order_count)
        ...
        apply_billing_group_fills(sheet, group_fills)
        ...
        table.tableStyleInfo = TableStyleInfo(name="TableStyleMedium4", showRowStripes=False, showFirstColumn=False, showLastColumn=False)
...
def export_active_billing_report(
    ...
    for group_index, (ref_date, row) in enumerate(selected):
        orders = billing_order_numbers(row)
        order_count = max(1, len(orders))
        first_row = detail.max_row + 1
        ...
        for order_index, order_number in enumerate(orders):
            detail.append([
                ...
                split_billing_value(row.get("Grand Total Shipment Ftp Wgt Kg"), order_index, order_count),
                split_billing_value(row.get("Grand Total Shipment Ftp Vol m3"), order_index, order_count),
                split_billing_value(base, order_index, order_count),
                *[
                    split_billing_value(extra_amounts.get(label, ""), order_index, order_count)
                    for label in active_extra_columns
                ],
                split_billing_value(active, order_index, order_count),
                clean_text(row.get("Extra Attivi Applicati")),
                clean_text(row.get("Tariffa Attiva Applicata")),
                clean_text(row.get("Stato")),
            ])
        active_group_fills.append((first_row, detail.max_row, billing_group_fill(group_index)))
    ...
    apply_billing_group_fills(detail, active_group_fills)
    ...
    detail_table.tableStyleInfo = TableStyleInfo(name="TableStyleMedium4", showRowStripes=False, showFirstColumn=False, showLastColumn=False)
...
PLANNED_EXPORT_EXCLUDED_COLUMNS = {
    "Costo Attivo",
    "Costo Passivo",
    "Extra BRT Totale",
    "Margine",
...
PLANNED_EXPORT_NUMERIC_COLUMNS = {
    "Theoretical Pallets",
    "Pallet Manuali",
    "Pallet Fatturati",
    "Grand Total Shipment Ftp Wgt Kg",
}
PLANNED_EXPORT_MONEY_COLUMNS = {
    "Extra Attivi Totale",
}


def export_planned_shipments(
    ...
    for row in selected_rows:
        ...
        for column in columns:
            if column in PLANNED_EXPORT_NUMERIC_COLUMNS or column in PLANNED_EXPORT_MONEY_COLUMNS:
                number = to_float(row.get(column))
                values.append(number if number is not None else "")
            elif column in DATE_ONLY_COLUMNS:
                values.append(parse_date_value(row.get(column)) or display_value(row, column))
            else:
        ...
        for column_cells in sheet.iter_cols(min_col=index + 3, max_col=index + 3, min_row=2):
            for cell in column_cells:
                if isinstance(cell.value, date):
                    cell.number_format = "dd/mm/yyyy"
    table_ref = f"A1:{get_column_letter(sheet.max_column)}{sheet.max_row}"
    table = Table(displayName="SpedizioniPianificate", ref=table_ref)
...
""".strip(),
    ...
    def mark_selected_delivered(self) -> None:
        ...
        try:
            delivered_at = ""
            for shipment in selected_shipments:
                delivered_at, shipment_xml_paths = mark_delivered(shipment)
                xml_paths.extend(shipment_xml_paths)
        except Exception as exc:
