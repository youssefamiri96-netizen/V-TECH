    ...
    with sqlite3.connect(db_path) as conn:
        ...
        for row in shipments:
            ...
            if status == STATUS_PLANNED and not clean_text(planned_at):
                planned_at = date.today().strftime("%Y-%m-%d")
...
def mark_planned(shipment: str, planned_at: str | None = None, db_path: Path = DB_PATH) -> None:
    planned_value = clean_text(planned_at) or datetime.now().strftime("%Y-%m-%d")
    init_db(db_path)
    with sqlite3.connect(db_path) as conn:
        conn.execute(
...
<html>
  ...
  <body>
    ...
    <div class="summary">
      <strong>Data mail:</strong> {html_lib.escape(date_text)}<br>
