...
def generate_delivery_xml_for_order(row: dict[str, Any], delivered_at: str, order_number: str) -> Path:
    ...
    output_path = XML_DIR / f"KN_{safe_xml_token(order_number)}.xml"
    ...
    event_date.text = f"{delivery_date.strftime('%Y%m%d')}180000"
...
def generate_delivery_xml(row: dict[str, Any], delivered_at: str) -> list[Path]:
    return [
        generate_delivery_xml_for_order(row, delivered_at, order_number)
        for order_number in delivery_order_numbers(row)
    ]
