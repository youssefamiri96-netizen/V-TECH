...
def mark_delivered(shipment: str, db_path: Path = DB_PATH) -> tuple[str, list[Path]]:
    ...
    xml_paths = generate_delivery_xml(row, delivered_at)
    init_db(db_path)
    with sqlite3.connect(db_path) as conn:
        conn.execute(
            "UPDATE shipments SET status = ?, delivered_at = ?, delivery_xml_path = ? WHERE shipment = ?",
            (STATUS_DELIVERED, delivered_at, " | ".join(str(path) for path in xml_paths), shipment),
        )
    return delivered_at, xml_paths
