...
AVAILABLE_CARRIERS = ["BRT", "KN", "GRENDI", "GD TRASPORTI", "FERCAM", "DACHSER", "DHL", "GEODIS", "DSV", "ALTRO"]
EMPTY_FILTER_VALUE = "__EMPTY__"
DATE_ONLY_COLUMNS = {"Late Ship Date", "Early Delivery Date", "Data Ship Minima SLA", "Prima Consegna SLA", "Data Partenza", "Data Partenza Wave", "Data Scarico Prenotato"}
...
def merge_duplicate_order_shipments(db_path: Path = DB_PATH) -> int:
    ...
    with sqlite3.connect(db_path) as conn:
        rows = conn.execute(
            """
            SELECT shipment, orders_text, status, departed_at, unload_date, unload_time, unload_booking_ref,
            ...
            groups.setdefault(key, []).append(
                {
                    ...
                    "departed_at": clean_text(item[3]),
                    "unload_date": clean_text(item[4]),
                    "unload_time": clean_text(item[5]),
                    "unload_booking_ref": clean_text(item[6]),
                    "delivered_at": clean_text(item[7]),
                    "delivery_xml_path": clean_text(item[8]),
                    "source_file": clean_text(item[9]),
                    "imported_at": clean_text(item[10]),
                    "payload_json": item[11],
                    "manual_carrier": clean_text(item[12]),
                    "manual_passive_cost": item[13],
                    "manual_service_level": clean_text(item[14]),
                    "manual_pallets": item[15],
                }
            )
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        existing_rows = conn.execute(
            ...
            SELECT shipment, orders_text, status, delivered_at, manual_carrier,
                   manual_passive_cost, manual_service_level, manual_pallets, unload_date,
                   unload_time, unload_booking_ref
            FROM shipments
            """
        ).fetchall()
        ...
        for row in shipments:
            ...
            manual_pallets = (
                existing[7]
                if existing and existing[7] is not None
                else to_float(row.get("Pallet Manuali"))
            )
            unload_date = existing[8] if existing and clean_text(existing[8]) else clean_text(row.get("Data Scarico Prenotato"))
            unload_time = existing[9] if existing and clean_text(existing[9]) else clean_text(row.get("Ora Scarico Prenotato"))
            unload_booking_ref = existing[10] if existing and clean_text(existing[10]) else clean_text(row.get("Riferimento Booking Scarico"))
            if manual_carrier:
            ...
            conn.execute(
                ...
                INSERT INTO shipments (
                    ...
                    manual_service_level, service_level, freight_code, booking_status, province, customer, active_cost, passive_cost,
                    manual_passive_cost, manual_pallets, margin,
                    status, unload_date, unload_time, unload_booking_ref, delivered_at, source_file, imported_at, payload_json
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ...
        for shipment in shipment_set:
            ...
            conn.execute(
                ...
                INSERT INTO shipments (
                    ...
                    manual_service_level, service_level, freight_code, booking_status, province, customer, active_cost, passive_cost,
                    manual_passive_cost, manual_pallets, margin,
                    status, departed_at, unload_date, delivered_at, delivery_xml_path, source_file, imported_at, payload_json
                )
                ...
                (
                    ...
                    clean_text(payload.get("Prenotazione Scarico")),
...
def mark_planned(shipment: str, db_path: Path = DB_PATH) -> None:
    init_db(db_path)
    with sqlite3.connect(db_path) as conn:
        conn.execute(
            "UPDATE shipments SET status = ?, departed_at = NULL, unload_date = NULL, delivered_at = NULL, delivery_xml_path = NULL WHERE shipment = ?",
            (STATUS_PLANNED, shipment),
        )
...
def mark_departed(shipment: str, departed_at: str | None = None, db_path: Path = DB_PATH) -> None:
    departed_value = departed_at or datetime.now().strftime("%Y-%m-%d")
    init_db(db_path)
    with sqlite3.connect(db_path) as conn:
        conn.execute(
            "UPDATE shipments SET status = ?, departed_at = ?, unload_date = NULL, delivered_at = NULL, delivery_xml_path = NULL WHERE shipment = ?",
            (STATUS_DEPARTED, departed_value, shipment),
        )
...
def get_shipment_from_db(shipment: str, db_path: Path = DB_PATH) -> dict[str, Any] | None:
    ...
    with sqlite3.connect(db_path) as conn:
        row = conn.execute(
            """
            SELECT payload_json, status, departed_at, unload_date, unload_time, unload_booking_ref,
                   delivered_at, delivery_xml_path,
                   manual_carrier, manual_passive_cost, manual_service_level, manual_pallets
            FROM shipments
            WHERE shipment = ?
            """,
            (shipment,),
    ...
    if not clean_text(payload.get("Early Delivery Date")):
...
def refresh_early_delivery_dates_from_source(vtech_path: Path | None, db_path: Path = DB_PATH) -> int:
    ...
    for shipment, payload_json in db_rows:
        ...
        if any(not clean_text(payload.get(field)) for field in source_fields):
...
def set_unload_date(
    ...
    if clean_text(row.get("Stato")) != STATUS_CONFIRMED:
        raise ValueError("La data scarico si imposta solo sulle spedizioni in FTL CONFERMATI.")

    normalized = "" if clear or not clean_text(unload_date) else normalize_unload_date(unload_date)
    row["Data Scarico Prenotato"] = normalized
    ...
    for carrier, carrier_rows in sorted(by_carrier.items()):
        ...
        for ref_date, row in carrier_rows:
            sheet.append([
                ...
                to_float(row.get("Extra BRT Totale")) or 0,
        ...
        _style_passive_sheet(sheet, money_columns=[13, 14, 15], numeric_columns=[10, 11, 12])
        table = Table(displayName=_safe_table_name("Passivo", carrier, used_tables), ref=f"A1:Q{sheet.max_row}")
...
def export_active_billing_report(
    ...
    for cell in detail["A"][1:]:
        if isinstance(cell.value, date):
            cell.number_format = "dd/mm/yyyy"
    _style_passive_sheet(detail, money_columns=[15, 16, 17], numeric_columns=[12, 13, 14])
    detail_table = Table(displayName="DettaglioAttivo", ref=f"A1:T{detail.max_row}")
...
def load_shipments_from_db(db_path: Path = DB_PATH) -> list[dict[str, Any]]:
    ...
    with sqlite3.connect(db_path) as conn:
        db_rows = conn.execute(
            """
            SELECT payload_json, status, departed_at, unload_date, delivered_at, freight_code, booking_status,
                   delivery_xml_path, manual_carrier, manual_passive_cost, manual_service_level, manual_pallets
            FROM shipments
    ...
    for (
        ...
        if booking_status and not clean_text(row.get("Prenotazione Scarico")):
        ...
        if not clean_text(row.get("Early Delivery Date")):
...
class VTechApp(tk.Tk):
    ...
    def _build_groupage_mail_bar(self, parent: ttk.Frame) -> None:
        ...
        tk.Label(
            copy,
            text="Crea una bozza Outlook con i groupage pianificati la cui Wave contiene la data indicata.",
            bg=SURFACE_BG,
            fg=MUTED_COLOR,
            font=("Segoe UI", 9),
            anchor="w",
        ...
        date_entry = ttk.Entry(controls, textvariable=self.groupage_mail_date_var, width=12)
        date_entry.grid(row=1, column=0, sticky="ew", padx=(0, 8))
        date_entry.bind("<Return>", lambda _event: self.create_groupage_outlook_mail())
        self._action_button(
            controls,
            "Calendario",
            SURFACE_SOFT,
            TEXT_COLOR,
            self.open_groupage_mail_calendar,
    ...
    def _groupage_mail_rows(self, target_date: date | None = None) -> list[dict[str, Any]]:
        mail_date = target_date or date.today()
        rows = [
            row for row in self.shipments
            if clean_text(row.get("Stato")) == STATUS_PLANNED
            and self._is_brt_groupage(row)
            and self._wave_contains_date(row.get("Wave"), mail_date)
        ]
        return sorted(
            rows,
            key=lambda row: (
...
<html>
  ...
  <body>
    ...
    <div class="summary">
      <strong>Data ritiro:</strong> {html_lib.escape(date_text)}<br>
...
""".strip(),
    ...
    def create_groupage_outlook_mail(self) -> None:
        try:
            target_date = self._parse_groupage_mail_date()
        except ValueError as exc:
            messagebox.showerror("Data non valida", str(exc))
            return

        rows = self._groupage_mail_rows(target_date)
        if not rows:
            tokens = ", ".join(sorted(self._wave_tokens_for_date(target_date)))
            messagebox.showinfo(
                "Nessuna spedizione",
                "Non ci sono spedizioni groupage pianificate con Wave contenente "
                f"la data selezionata ({tokens}).",
            )
            return

        subject = f"Ritiri groupage V-Tech - {target_date.strftime('%d/%m/%Y')}"
        html_body = self._build_groupage_mail_html(rows, target_date)
        try:
