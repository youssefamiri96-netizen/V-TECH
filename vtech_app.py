...
from tariff_engine import apply_contract_sla_to_shipments, apply_tariffs_to_shipments
...
AVAILABLE_CARRIERS = ["BRT", "KN", "GRENDI", "FERCAM", "DACHSER", "DHL", "GEODIS", "DSV", "ALTRO"]
...
def merge_duplicate_order_shipments(db_path: Path = DB_PATH) -> int:
    ...
    with sqlite3.connect(db_path) as conn:
        rows = conn.execute(
            """
            SELECT shipment, orders_text, status, departed_at, unload_date, delivered_at, delivery_xml_path,
            ...
            groups.setdefault(key, []).append(
                {
                    ...
                    "delivered_at": clean_text(item[5]),
                    "delivery_xml_path": clean_text(item[6]),
                    "source_file": clean_text(item[7]),
                    "imported_at": clean_text(item[8]),
                    "payload_json": item[9],
                    "manual_carrier": clean_text(item[10]),
                    "manual_passive_cost": item[11],
                    "manual_service_level": clean_text(item[12]),
                    "manual_pallets": item[13],
                }
            )
        ...
        for row in shipments:
            ...
            conn.execute(
                ...
                INSERT INTO shipments (
                    ...
                    status, unload_date, delivered_at, source_file, imported_at, payload_json
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
def get_shipment_from_db(shipment: str, db_path: Path = DB_PATH) -> dict[str, Any] | None:
    ...
    with sqlite3.connect(db_path) as conn:
        row = conn.execute(
            """
            SELECT payload_json, status, departed_at, unload_date, delivered_at, delivery_xml_path,
...
def load_shipments_from_db(db_path: Path = DB_PATH) -> list[dict[str, Any]]:
    ...
    if rows and (active_rates_path or brt_passive_path):
        apply_tariffs_to_shipments(
            rows,
